...
Before presenting detailed results, we analyze the precision-recall tradeoff to justify the choice of \texttt{max\_claims=5} as the primary configuration. The system was evaluated across six configurations with \texttt{max\_claims} $\in \{1, 3, 5, 7, 10, 15\}$.
...
The high standard deviation (0.334) in verdict accuracy warrants investigation. Analysis of per-video results reveals a bimodal distribution:

\begin{itemize}
    \item \textbf{14 videos (46.7\%)} achieved 100\% verdict accuracy---all extracted claims were correctly classified.
    \item \textbf{9 videos (30.0\%)} achieved 0\% verdict accuracy---no claims were correctly classified.
    \item \textbf{7 videos (23.3\%)} achieved partial accuracy between 20\% and 80\%.
\end{itemize}

\paragraph{Root Cause: Evidence Retrieval Correlation}

Detailed analysis reveals a near-perfect correlation between evidence retrieval success and verdict accuracy. Table~\ref{tab:accuracy-retrieval-correlation} presents representative examples demonstrating this relationship.

\begin{table}[H]
\centering
\caption{Correlation between evidence retrieval and verdict accuracy}
...
\midrule
\multicolumn{4}{l}{\textit{High accuracy (100\%)}} \\
\quad Brain Benefits of Exercise (TED) & 100\% & 100\% & 2.5 \\
\quad Climate Change (NatGeo) & 100\% & 100\% & 2.3 \\
\quad Fossil Fuels & 100\% & 100\% & 2.7 \\
\quad Fluoride \& IQ Claims & 100\% & 100\% & 2.1 \\
\midrule
\multicolumn{4}{l}{\textit{Zero accuracy (0\%)}} \\
\quad FBI \& January 6th & 0\% & 0\% & 0.0 \\
\quad Gender Wage Gap & 0\% & 0\% & 0.0 \\
\quad Egg Prices (Political) & 0\% & 0\% & 0.0 \\
\quad Immigration Statistics & 0\% & 0\% & 0.0 \\
\bottomrule
\end{tabular}
\end{table}

When evidence retrieval fails completely (0\% success rate), the system defaults all verdicts to UNCLEAR due to insufficient evidence, which systematically mismatches ground truth labels. Conversely, videos with successful evidence retrieval consistently achieve 100\% verdict accuracy, indicating that the LLM's reasoning capabilities are effective when provided with relevant evidence.

\paragraph{Video Characteristics Affecting Retrieval}

Analysis suggests that politically current or contentious topics exhibit higher retrieval failure rates. Videos about contested political claims (election procedures, wage gap debates, recent policy events) showed 0\% retrieval success, while scientific and health topics generally achieved successful retrieval. Potential contributing factors include:

\begin{itemize}
    \item \textbf{Temporal sensitivity}: Claims about recent events may lack stable, indexed web sources at the time of search.
    \item \textbf{Web scraping limitations}: Certain news sites or platforms may block automated content extraction.
    \item \textbf{Query formulation challenges}: Politically charged claims may generate queries that return opinion pieces rather than factual sources.
\end{itemize}

This finding identifies evidence retrieval robustness as the primary bottleneck for system improvement, rather than LLM reasoning quality.
...
\textbf{Metric} & \textbf{Value} \\
\midrule
Evidence retrieval success rate & 60.7\% \\
Average sources per query & 1.52 \\
Average evidence items per claim & 3.04 \\
\bottomrule
\end{tabular}
\end{table}

The evidence retrieval success rate of 60.7\% indicates that approximately 6 out of 10 search queries return usable evidence. When evidence is retrieved, claims receive an average of 3.04 evidence items, providing multiple perspectives for verdict synthesis.
...
\begin{table}[H]
\centering
\caption{Source reliability distribution (n=456 total sources)}
...
The overwhelming majority of retrieved sources (84.0\%) receive high reliability ratings from the Media Bias/Fact Check-based assessment system \citep{mbfc2024methodology}. No sources received low reliability ratings, and only 1.3\% were classified as unknown (typically due to missing MBFC data for niche domains). This distribution validates the effectiveness of the source credibility filtering, which prioritizes established, factual reporting sources.
...
Analysis of verdict errors reveals systematic patterns. The most common failure mode occurs when evidence retrieval fails entirely, causing the system to default to UNCLEAR verdicts regardless of ground truth. Table~\ref{tab:verdict-errors} categorizes the primary error types.
...
The predominant error pattern (evidence retrieval failure $\rightarrow$ UNCLEAR verdict) suggests that improving search query generation and web scraping robustness would yield substantial accuracy gains. Videos achieving 0\% verdict accuracy consistently exhibited evidence retrieval success rates near 0\%, while videos achieving 100\% accuracy consistently had success rates near 100\%.

\subsubsection{Challenging Video Categories}

Certain video categories proved more challenging for the system:

\textbf{Controversial topics with contested evidence:} Videos on politically charged topics (e.g., wage gap, climate policy) often retrieved conflicting evidence from sources with different editorial perspectives, leading to MIXED or UNCLEAR verdicts even when ground truth indicated clearer stances.

\textbf{Niche or specialized domains:} Videos on highly specialized topics sometimes failed evidence retrieval entirely due to limited web coverage of specific claims.

\textbf{Conspiracy content:} Videos promoting misinformation (e.g., chemtrails, geoengineering conspiracies) were correctly identified as containing false claims when evidence was retrieved, demonstrating the system's capability to fact-check problematic content.
...
These factors explain part of the bimodal verdict accuracy distribution: search failures are not necessarily reproducible, and a claim that failed evidence retrieval in one run might succeed in another.
...
The experimental evaluation demonstrates that Factible achieves its design goals of high-precision claim extraction with evidence-backed verification:

\begin{enumerate}
    \item \textbf{High-precision claim extraction:} The system extracts claims with 81.3\% precision, meaning users can trust that presented claims are legitimate factual statements from the video.

    \item \textbf{Effective importance prioritization:} The MAP score of 0.870 and Recall@Important of 39.5\% confirm that the system successfully identifies and prioritizes the most important claims, capturing nearly 40\% of high-importance claims while extracting only 30\% of total claims.

    \item \textbf{Reliable evidence sourcing:} 84.0\% of retrieved evidence comes from high-reliability sources, ensuring that verdicts are grounded in credible information.

    \item \textbf{Meaningful verdict accuracy:} The 73.3\% verdict accuracy substantially exceeds random baselines and provides evidence-backed explanations enabling user evaluation.

    \item \textbf{Practical efficiency:} Mean processing time of 129.6 seconds per video enables near-interactive use at negligible API cost.

    \item \textbf{Identified improvement opportunities:} The bimodal verdict accuracy distribution suggests that enhancing evidence retrieval robustness would yield significant overall accuracy gains.
\end{enumerate}

These results position Factible as a viable tool for preliminary fact-checking of YouTube content, providing high-precision claim identification and evidence-backed verification suitable for users seeking to evaluate video claims.
...
This appendix provides details on the ground truth annotation dataset used for system evaluation. The complete dataset comprises 30 YouTube videos with 504 annotated claims across three thematic categories.
