...
\begin{enumerate}
    ...
    \item \textbf{LLM usage optimization}: \textbf{Achieved}. The system uses GPT-4o-mini effectively with deterministic settings and token budgets. Cost, latency, and quality trade-offs were analyzed, and prompt optimization strategies with structured outputs were implemented across all pipeline components.
...
\begin{enumerate}
    \item \textbf{Prompt engineering is critical but brittle}: Small changes in prompt wording can significantly affect LLM outputs. The claim prioritization approach required extensive iteration to achieve stable, high-quality claim extraction.
