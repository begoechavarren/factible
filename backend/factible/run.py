...
from factible.components.online_search.search import search_online, search_online_async
...
async def run_factible_async(
    ...
    *,
    ...
    """
    Async version of run_factible with parallelization at 3 levels:
    ...
    *,
    ...
    with ExperimentTracker(
        ...
        if not transcript_text.strip():
            ...
            empty_claims = ExtractedClaims(claims=[], total_count=0)
            return asyncio.run(generate_run_output(empty_claims, [], transcript_data))
        ...
        with timer("Step 2: Claim extraction"):
            extracted_claims = asyncio.run(
                extract_claims(transcript_text, max_claims=max_claims)
            )
        ...
        if not claims_to_process:
            ...
            return asyncio.run(
                generate_run_output(processed_claims, [], transcript_data)
            )
...
if __name__ == "__main__":
    ...
    VIDEO_URL = "https://www.youtube.com/watch?v=BJWq1FeGpCw"
